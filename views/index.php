...
$destinations = $manage->getAllDestination();
?>
...
<main class="max-w-4xl mx-auto px-4 pt-6 pb-10 text-slate-900">
    <div class="mb-6">
        <form action="destination.php" method="GET" class="w-full">
            <label for="user-search" class="block text-sm font-medium text-slate-700 mb-1">
            ...
            <div class="relative">
                <input
                    ...
                    type="text"
                    autocomplete="off"
                    placeholder="Rome, Tunis, New York..."
                    class="w-full rounded-full bg-white border border-slate-300 px-4 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
                <i class="fa-solid fa-magnifying-glass absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>

                <div id="user-search-results"
                    class="hidden absolute left-0 right-0 mt-2 rounded-xl bg-white shadow-lg overflow-hidden z-50 border border-slate-100">
                </div>
            </div>
        </form>
    </div>
        ...
        (function() {
            ...
            function render(results) {
                if (!Array.isArray(results) || results.length === 0) {
                    box.innerHTML = '<div class="px-3 py-2 text-sm text-neutral-400">No result</div>';
                ...
                box.innerHTML = results.map(r => {
                    ...
                    const href = `./destination.php?destination=${encodeURIComponent(r.location)}`;
                    return `
                    <a href="${href}" class="block px-3 py-2 hover:bg-slate-50">
                        <div class="text-sm text-slate-900 truncate">${location}</div>
                    </a>
                `;
                }).join('');
                box.classList.remove('hidden');
            ...
            async function search(q) {
                ...
                try {
                    box.classList.remove('hidden');
                    box.innerHTML = '<div class="px-3 py-2 text-sm text-neutral-400">Searching…</div>';

                    const res = await fetch(`../process/research.php?q=${encodeURIComponent(q)}`, {
                    ...
                    box.innerHTML = '<div class="px-3 py-2 text-sm text-amber-500">Search error</div>';
                    box.classList.remove('hidden');
                }
            }
                        ...
                        <div class="space-y-2">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="text-right min-w-[90px]">
                                    <?php if ($isPremium): ?>
                                        <p class="yellow text-base sm:text-lg font-bold leading-tight">
                                            <?= number_format((float) $price, 2, ',', ' ') ?> $
                                        </p>
                                    <?php else: ?>
                                        <p class="vert text-base sm:text-lg font-bold leading-tight">
                                            <?= number_format((float) $price, 2, ',', ' ') ?> $
                                        </p>
                                    <?php endif; ?>
